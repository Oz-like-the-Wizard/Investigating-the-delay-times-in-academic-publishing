...
class SciHub_HydroPump(object):
    ...
    def rec_date_capture(self,pdf_read):
        rec_patterns = [SciHub_HydroPump.rec_1, SciHub_HydroPump.rec_2, SciHub_HydroPump.rec_3, SciHub_HydroPump.rec_4]
    ...
    def acc_date_capture(self,pdf_text):
        acc_patterns = [SciHub_HydroPump.acc_1, SciHub_HydroPump.acc_2, SciHub_HydroPump.acc_3, SciHub_HydroPump.acc_4]
    ...
    def get_pdf_bytes(self, identifier):
        ...
        try:
            ...
            raise OgiError("cant_connect_to_Scihub")
        ...
        try:
            ...
            if ("article not found" in s.text) |  ("sci-hub has not included this article yet" in s.text):
                return "article_not_in_Scihub", "article_not_in_Scihub"
    ...
    def get_dates(self, identifier):
        try:
            ...
            if pdf_bytes == "article_not_in_Scihub":
                return "article_not_in_Scihub"
    ...
    def date_finder(self,pdf_read):
        rec_date = self.rec_date_capture(pdf_read)
        acc_date = self.acc_date_capture(pdf_read)
        return [rec_date, acc_date] if (rec_date != None) | (acc_date != None) else "no_date_found"


    def get_dates_kws(self, identifier):
        try:
            pdf_url, pdf_bytes = self.get_pdf_bytes(identifier)
            if pdf_bytes == "article_not_in_Scihub":
                return "article_not_in_Scihub", "article_not_in_Scihub", "article_not_in_Scihub"

        except OgiError:
            return "direct_url_error", "direct_url_error", "direct_url_error" 

        except FourZeroFourError:
            return "pdf_bytes_error", "pdf_bytes_error", "pdf_bytes_error"
